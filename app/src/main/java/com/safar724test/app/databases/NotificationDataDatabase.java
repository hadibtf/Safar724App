...
@Database(entities = NotificationData.class, exportSchema = false, version = 1)
public abstract class NotificationDataDatabase extends RoomDatabase {
    ...
    public static synchronized NotificationDataDatabase getInstance(Context context) {
        if (instance == null) {
            instance = Room.databaseBuilder(context.getApplicationContext(), NotificationDataDatabase.class, DB_NAME)
